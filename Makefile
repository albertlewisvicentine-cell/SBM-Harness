...
# Compiler and flags
CC = gcc
CFLAGS = -std=c11 -Wall -Wextra -O2 -Iinclude -Itests/c
...
$(BUILD_DIR)/%.o: src/c/%.c include/sbm_harness.h include/sbm_types.h include/sbm_snapshot.h | $(BUILD_DIR)
	$(CC) $(CFLAGS) -c $< -o $@

# Build fault injection test suite
$(FAULT_INJECTION): tests/c/fault_injection.c $(OBJ_FILES) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $^
...
# Build unit tests
$(UNIT_TESTS): tests/c/unit_tests.c $(OBJ_FILES) | $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $@ $^
...
repro-check: $(SIM_C)
	...
	PYTHONPATH=src python3 -m sbm_harness.simulation --seed $(REPRO_SEED) --out $(BUILD_DIR)/repro/py_trace.jsonl
...
safety-gate:
	...
	PYTHONPATH=src python3 -m sbm_harness.safety_gate $(BUILD_DIR)/repro/results.jsonl --p_max 0.01
	@echo "=== Safety gate passed ==="

# Run all tests
.PHONY: test
test: $(BUILD_DIR) $(UNIT_TESTS) $(FAULT_INJECTION)
